...
class Category(Base):
    ...
    products:Mapped[list["Product"]] = relationship(secondary=ProductsAssoiation,back_populates='categories')
...
class Product(Base):
    ...
    orders:Mapped[list['Order']] = relationship(secondary=OrdersAssoiation,back_populates="products")
    categories:Mapped[list["Category"]] = relationship(secondary=ProductsAssoiation,back_populates="products")
...
class Comment(Base):
    ...
    is_verified:Mapped[bool] = mapped_column(default=False) # verified by admin to show
...
class Order(Base):
    ...
    products:Mapped[list['Product']] = relationship(secondary=OrdersAssoiation,back_populates="orders")
...
def create_new_user(name:str,email:str,phone_number:int,username:str,password:str,is_active:bool=None,join_date:datetime.datetime=None):
    password = create_password_hash(password)
    user = User(name = name,email = email,phone_number = phone_number,username = username,password = password,is_active = is_active,join_date = join_date,
    )
    with Session(engine) as ses,ses.begin():
        ses.add(user)
    return user

def new_product(session:Session,name:str,stock:int,price:int,details:str,categories:list[Category],orders:list[Order]=None):
