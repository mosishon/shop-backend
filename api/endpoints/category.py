...
router = APIRouter(prefix="/category",tags=['category'],dependencies=[Depends(verify_user)])
...
async def get_category_by_id(category_id:int):
    with Session(engine) as ses:
        ...
        if not category: #Order is not for this user or order not exists
            return JSONResponse({"code":"NOT_FOUND","error":"category not found"},404)
...
async def get_all_categories(limit:int=10,offset:int=0):
    ...
    with Session(engine) as ses:
        ...
        categories = [PydanticCategory(slug=category.slug,name=category.name) for category in cats]
